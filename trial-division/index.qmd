...
Results show improving performance but with diminishing returns as wheel size increases. If we increase wheel size too much, the code no longer fits in icache and performance degrades drastically. Practical implementations usually pick a fixed wheel size such as 210 or 2310, small enough to make this unlikely.
...
or is prime. So all we have to is precompute a list of primes at least up to that value,
and pass it in:
...
def trial_division_many_segmented_sieve(n_list):
    # Start with largest value n for each factor, search
    # for a smaller one
    results = n_list
    ...
    for p_min in range(0, sieve_max, segment_size):
        p_max = p_min + segment_size - 1
        primes_segment = get_primes_between(p_min, p_max, sieve_primes)
        for i in range(0, len(n_list)):
            for p in primes_segment:
                if results[i] > p and n_list[i] % p == 0:
                    results[i] = p
                    break

    return results

n_list = [random.randint(1, 999_000_000_000) for _ in range(100)]
