...
* If 1 < *a* < *n* and *a*^n^ % *n* ≠ *a*, then *n* is composite.

For example, 2^15^ % 15 = 32768 % 15 = 8 ≠ 2, showing 15 is composite. Note that this doesn't tell us any factor of 15, only that it *has* factors.

The idea is that if we pick many random values of *a* and find *a*^n^ % *n* = *a* for all of them, we can have high confidence that *n* is prime:
...
def is_prime_fermat(n):
    for i in range(20):
        ...
        if a**n % n != a:  # Slow!
            return False
    return True  # Wrong result for Carmichael numbers!

for n in [11, 13, 15, 561]:
    print(f"{n} prime: {is_prime_fermat(n)}")
```

There are two problems with this: 1. computing *a*^n^ is really expensive for large *n*; 2. there are certain composite *n* called Carmichael numbers (561, 1105, ...) where this test incorrectly returns `True`. We'll address both.

### Speeding up exponentiation mod n
